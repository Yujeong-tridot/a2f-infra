...
│  │  (t3.large × 2)                │    │  (g4dn.xlarge × 2)                 │ │
│  │                                │    │                                    │ │
│  │  ┌──────────┐ ┌──────────┐     │    │  ┌─────────────┐ ┌─────────────┐  │ │
│  │  │ backend  │ │  tarot   │     │    │  │ a2f-grpc    │ │ a2f-grpc    │  │ │
│  │  │  :4008   │ │  :4007   │     │    │  │ server-0    │ │ server-1    │  │ │
│  │  └──────────┘ └──────────┘     │    │  │ :50051      │ │ :50051      │  │ │
│  │  ┌──────────┐ ┌──────────┐     │    │  │ (GPU Node1) │ │ (GPU Node2) │  │ │
│  │  │custom_cha│ │blind_date│     │    │  └─────────────┘ └─────────────┘  │ │
│  │  │  :4009   │ │  :4010   │     │    │                                    │ │
│  │  └──────────┘ └──────────┘     │    │  StatefulSet (replicas: 2)         │ │
│  │  ┌──────────┐ ┌──────────┐     │    │  PDB: minAvailable 1              │ │
│  │  │concierge │ │fact_worke│     │    │  GPU: T4 16GB per node             │ │
│  │  │  :4011   │ │ (worker) │     │    │  CUDA 12.8+ / TensorRT 10.8+       │ │
│  │  └──────────┘ └──────────┘     │    │                                    │ │
│  └────────────────────────────────┘    └────────────────────────────────────┘ │
│                │                                      │                       │
│                └──────────────┬───────────────────────┘                       │
...
│   └── a2f-grpc-server/       # GPU gRPC 서버 (StatefulSet)
│
...
| `avadot-node-gpu` | g4dn.xlarge   | 2     | GPU 추론   | a2f-grpc-server (StatefulSet × 2, PDB minAvailable: 1)          |
...
| Service | Port | Type | Replicas | Description |
|---------|------|------|----------|-------------|
| a2f-grpc-server | 50051 | StatefulSet | 2 | NVIDIA Audio2Face gRPC 서버 (PDB minAvailable: 1) |
